// ===== Loyalsoldier 全局覆写脚本 v5.2（精简稳定版·手动选点）=====
// [v5.2] 取消自动测速与节点漂移: 代理组改为手动 select, 你自己选节点, 不再自动切换。
//   (后台不再并发测 300+ 个节点, 也就不会再出现 50ms→3000ms 那种测速假跳变。)
// 思路: ①只用 L大规则 ②直连走 DHCP 系统 DNS ③境外走谷歌 DNS(经代理) ④单组自动选最快
// [v5.1 稳定性] 针对"同一节点要刷几次才能打开":
//   - tcp-concurrent: 多 IP 并发拨号, 哪个先通用哪个 → 大幅减少首连超时
//   - unified-delay: 统一延迟口径, 测速更准, 不会选到虚假低延迟的坏节点
...
function main(config) {
    ...
    // ---- 0. 稳定性开关: 并发建连 + 统一延迟 (减少"同一节点要刷几次"的首连失败) ----
    config["tcp-concurrent"] = true
    config["unified-delay"] = true

    // ---- 1. 代理组: 单组, 手动选择 (不自动测速、不漂移) ----
    config["proxy-groups"] = [
        {
            name: PROXY,
            type: "select",
