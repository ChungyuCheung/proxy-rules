// ===== Loyalsoldier 全局覆写脚本（v4.6 优化版）=====
// [v4.6 DNS 两桶模型] 直连(国内+公司+内网) → DHCP 系统 DNS(system://, 随网络自动切换);
//   境外/代理域名 → Google/Cloudflare DoH(8.8.8.8 / 1.1.1.1)。
...
function main(config) {
    ...
    // [境外] 默认解析器 = 8.8.8.8 / 1.1.1.1。用 IP 直连 DoH 形式(非明文 53),
    //   否则在国内会被劫持/污染。
    dns.nameserver = ["https://8.8.8.8/dns-query", "https://1.1.1.1/dns-query"]
    ...
    // [Bootstrap/节点] 代理服务器域名 & DoH 引导用国内 DNS 直接解析, 避免鸡生蛋。
